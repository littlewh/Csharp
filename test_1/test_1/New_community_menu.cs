...
namespace test_1
{
    public partial class New_community_menu : NForm
    {
        string admin_name;
        public New_community_menu(string admin_name)
        {
            InitializeComponent();
        ...
        private void New_community_menu_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                conn.Open();
                string query = "select user.uname,gamecommunity.gid,gamecommunity.gapplicant,gamecommunity.gname,gamecommunity.gflag from user,gamecommunity where gamecommunity.gapplicant = user.uid; ";
                ...
                while (reader.Read())
                {
                    string re_rid = reader["gid"].ToString();
                    string re_name = reader["uname"].ToString(); 
                    string re_topic = reader["gname"].ToString(); 
                    string re_id = reader["gapplicant"].ToString();
                    string re_flag = reader["gflag"].ToString();
                    if (re_flag == "0")
                    {
                        i++;
                        switch(i)
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (text != "")
            {
                ...
                this.Close();
                this.Dispose();
                new New_community(admin_name,rid).ShowDialog();//模态
