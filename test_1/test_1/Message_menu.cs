...
namespace test_1
{
    public partial class Message_menu : NForm
    {
        string admin_name;
        public Message_menu(string admin_name)
        {
            InitializeComponent();
        ...
        private void Message_menu_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                conn.Open();
                string query = "select user.uname,information.iid,information.uid,information.itopic,information.iflag from user,information where information.uid = user.uid; ";
                ...
                while (reader.Read())
                {
                    string re_rid = reader["iid"].ToString();
                    string re_name = reader["uname"].ToString(); 
                    string re_topic = reader["itopic"].ToString(); 
                    string re_id = reader["uid"].ToString();
                    string re_flag = reader["iflag"].ToString(); 
                    if (re_flag == "0")
                    {
                        i++;
                        switch(i)
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (text != "")
            {
                ...
                this.Close();
                this.Dispose();
                new Message(admin_name,rid).ShowDialog();//模态
